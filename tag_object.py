...
def tag_object(read_text):
    ...
    regex_mall2 = (r"((?!ห้างฉัตร)(ห้างสรรพสินค้า|ห้าง)((?!ร้าน|ดัง|เปิด|และ|ค้า|ได้|ซึ่ง|" + list_str_m + ")[ก-๙]{2,}))")
    ...
    regex_mall2 = (r"<mall2>((?!ห้างฉัตร)(ห้างสรรพสินค้า|ห้าง)((?!ร้าน|ดัง|เปิด|และ|ค้า|ได้|ซึ่ง|" + list_str_m + ")[ก-๙]{2,}))</mall2>")   
    regex_university2 = (r"<university2>((มหาวิทยาลัย|มหาลัย)(?!แล้ว|หลาย|ได้|เพื่อ|ตำบล|ชื่อดัง|ดังกล่าว|ที่เคย|ต้นสังกัด|ไม่|" + list_str_uni + ")([ก-๙]{2,}))</university2>")  
    ...
    ###### ส่วนที่โค้ชเขียน#############
        ...
        for i in x:
            word = i.text.strip('\"')
        #print (word)
        aof = ("สถานที่แก้แล้ว : <mall>"+word+"</mall>")
        print(aof)
    ...
    for matchNum, match in enumerate(matches_university2):
        ...
        for i in x:
            word = i.text
        #print (word)
        aof = ("สถานที่แก้แล้ว : <university>"+word+"</university>")
        print(aof)
    ...
    matches_act1 = regex.sub(regex_act1, r'<action1>\1</action1>', read_text)
    read_text = matches_act1
    matches_act2 = regex.sub(regex_act2, r'<action2>\1</action2>', read_text)
