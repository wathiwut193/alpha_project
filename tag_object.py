...
def tag_location(read_text):
    ...
    regex_all = (
        ...
        "(/)?(\d?\d?\d?)|((หมู่|หมู่ที่|(?<!ชั้น\s|\.)\sม\.)\s?(\d\d?))|((ที่|ใน|คา)(?<!พนักงาน)โรงแรม|โรงเรียน|โรงรับจำนำ|โรงงาน)(?!เถื่อน|เท่านั้น|ทั้ง|ต่างๆ|แห่งนี้|แต่|กับ|เพราะ|เป็น|ที่|ส่วน|สั่ง|แห่งเดียว|จะได้|เดียว|ได้สั่ง|ก็โดน|ใน|มี|ต่างหาก|ก็เห็น|ใกล้|ตนได้|และ|ว่า|ที่เกิดเหตุ)([ก-๙]{2,}(?=ชื่อ|ใน|ริม)|(?!จริง|ชื่อ|ดังกล่าว)[ก-๙]{2,})|"
