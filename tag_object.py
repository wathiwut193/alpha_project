...
def tag_secondary_action(read_text, n):
    if n == 1:
        regex_secondary_action = (
            r"((?!ตี\|?ตัว|หนี้|ตี้|ตี๋|ตบ\|?ตา|ตบ\|?ไฟ|ตี\|?ข่าว|ตี\|?ล้อม|ตี\|?กลับ|ตบะ)(ให้การ\|?ปฏิเสธ|ทะเลาะ\|?วิวาท|ล้วง\|?กระเป๋า|ชก\|?ต่อย|(?<!แ)ต่อย|ชิงทรัพย์|"
            "ลักทรัพย์|แบล็คเมล์|(?<!า|พื|นิ|โ|เว)ชก|วิ่งราว|ก่อ\|?เหตุ|ซ่อน\|?ตัว|ทำร้าย|ต่อสู้|ล่อลวง|ระเบิด|หลบหนี|มอบ\|?ตัว|สารภาพ|ยอม\|?รับ|ค้า\|?ยา|เสพ\|?ยา|ขายยา|"
            "กบดาน|ซ้อน|ปล้น|เตะ|ทุบ|(?<!เ)จี้|หลบ|(?<!ทวง\|?|เ)หนี|(?<!เ|ซิ|การัน|เข้า\|?บ้าน|โจม|ส)ตี(?!\|\s?\d)|(?<!เข|อุ|ฟุ|ผัก)ตบ))")
        read_text = regex.sub(regex_secondary_action, r'<กระทำรอง1>\1</กระทำรอง1>', read_text)
    elif n == 3:
        regex_secondary_action = (r'((ลาดตระเวน|แถลงการณ์|ลงพื้นที่|ยึดทรัพย์|รับแจ้ง|เปิดเผย|บุกรวบ|เผยว่า|'
                                  'ตามล่า|(?<!ส)ระบุ|ติดตาม|นำแถลง|บุก|(?<!เร่ง\|?)รวบ|ปะทะ|เก็บ|(?<!เวลา\|?)ไล่|(?<!ไม่\|?|หาก\|?)พบ))')
        read_text = regex.sub(regex_secondary_action, r'<กระทำรอง3>\1</กระทำรอง3>', read_text)
    return read_text
...
def tag_action(read_text):
    ...
    regex_act1 = (r"((?!ยิง\|?ตัว|ฆ่า\|?ตัว|ฟัน\|?กำไร)((?<!ไม่\|?)ยิง|(?<!ยา\|?)ฆ่า|ฆาตกรรม|(?<!อ\|?)สังหาร|กระทำ\|?ชำเรา|อนาจาร|ข่มขืน|ขืนใจ|ฉุด|ฟัน|แทง))")
    
    # regex_act2_f = (r"((" + list_str_act2 + "){e<=1})")
    regex_act2_f = (r"((ฆ่า\|?ตัว\|?ตาย|ยิง\|?ตัว\|?เอง|ปลิด\|?ชีพ\|?ตัว\|?เอง|ปลิด\|?ชีวิต\|?ตัว\|?เอง|จบ\|?ชีวิต(?!\|?ต่อ\|?ไป)|อัตวินิบาตกรรม){e<=1})")
    # regex_act2 = (r"<กระทำ2_ผิด>((\|)(" + list_str_act2 + "))</กระทำ2_ผิด>")
    regex_act2 = (r"<กระทำ2_ผิด>((\|)(ฆ่า\|?ตัว\|?ตาย|ยิง\|?ตัว\|?เอง|ปลิด\|?ชีพ\|?ตัว\|?เอง|ปลิด\|?ชีวิต\|?ตัว\|?เอง|จบ\|?ชีวิต(?!\|?ต่อ\|?ไป)|อัตวินิบาตกรรม))</กระทำ2_ผิด>")
    
    # regex_act3 = (r"((" + list_str_act3 + "))")
    regex_act3 = (r"(((?<!\.\|?)สอบสวน|ดำเนิน\|?การ\|?คดี|ดำเนิน\|?คดี|ลาดตระเวน|แถลง\|?การณ์|ลง\|"
    "?พื้นที่|ยึด\|?ทรัพย์|สอบ\|?ปากคำ|รับ\|?แจ้ง|ตรวจ\|?สอบ|เปิดเผย|ตรวจ\|?ค้น|ส่ง\|?ฟ้อง|"
    "วิสามัญ|เอา\|?ผิด|บุก\|?รวบ|เผย\|?ว่า|ตาม\|?ล่า|คุม\|?ตัว|ควบคุม|(?<!ชุด\|?|ฝ่าย\|?|ตำรวจ\|?)สืบสวน|"
    "ติดตาม|นำ\|?แถลง|รายงาน|วางแผน|จับกุม|(?<!หมาย\|?)จับ|นำ\|?ตัว|(?<!ส)ระบุ|ปะทะ|เก็บ|บุก|(?<!เร่ง\|?)รวบ(?!รวม)|"
    "(?<!เวลา\|?)ไล่|คุม|(?<!ไม่\|?|หาก\|?|สุภา\|?|กองทั\|?)พบ))")

    regex_act4 = (r"(((?!พาล|พานิช|พาชัย|พาย|พาหนะ|พาน|พาด)(?<!พก\|?|พัด\|?|สะ|วิ|ยาน|เฉ|บูร|อัม|อัล|อ|พิ|ทิ|อาคาร\|?)พา))")
    
    # regex_act5_f = (r"((?!ให้การ\|?ปฏิเสธ|ให้\|?กราด)(" + list_str_act5 + "){e<=1})")
    regex_act5_f = (r"((?!ให้การ\|?ปฏิเสธ|ให้\|?กราด)(ให้\|?ปาก\|?คำ|ให้\|?การ){e<=1})")
    # regex_act5 = (r"<กระทำ5_ผิด>((\|)(?!ให้การ\|?ปฏิเสธ|ให้\|?กราด)(" + list_str_act5 + "))</กระทำ5_ผิด>")
    regex_act5 = (r"<กระทำ5_ผิด>((\|)(?!ให้การ\|?ปฏิเสธ|ให้\|?กราด)(ให้\|?ปาก\|?คำ|ให้\|?การ))</กระทำ5_ผิด>")
    
    regex_act6_f = (r"(((?<!ป)รับ\|?สารภาพ){e<=1})")
    regex_act6 = (r"<กระทำ6_ผิด>((\|)((?<!ป)รับ\|?สารภาพ))</กระทำ6_ผิด>")

    # regex_act8_f = (r"((" + list_str_act8 + "){e<=1})")
    regex_act8_f = (r"((แจ้ง\|?ความ|ร้อง\|?ทุกข์|ฟ้อง\|?ร้อง|ร้อง\|?เรียน|แจ้ง\|?คดี){e<=1})")
    # regex_act8 = (r"<กระทำ8_ผิด>((\|?)((" + list_str_act8 + ")))</กระทำ8_ผิด>")
    regex_act8 = (r"<กระทำ8_ผิด>((\|?)((แจ้ง\|?ความ|ร้อง\|?ทุกข์|ฟ้อง\|?ร้อง|ร้อง\|?เรียน|แจ้ง\|?คดี)))</กระทำ8_ผิด>")

    # add tag
    matches_act1 = regex.sub(regex_act1, r'<กระทำ1>\1</กระทำ1>', read_text)
    read_text = matches_act1
    matches_act2_f = regex.sub(regex_act2_f, r'<กระทำ2_ผิด>\1</กระทำ2_ผิด>', read_text)
    read_text = matches_act2_f
    ...
    matches_act5_f = regex.sub(regex_act5_f, r'<กระทำ5_ผิด>\1</กระทำ5_ผิด>', read_text)
    read_text = matches_act5_f
    matches_act5 = regex.sub(regex_act5, r'<กระทำ5>\1</กระทำ5>', read_text)
    read_text = matches_act5
    matches_act6_f = regex.sub(regex_act6_f, r'<กระทำ6_ผิด>\1</กระทำ6_ผิด>', read_text)
    read_text = matches_act6_f
    matches_act6 = regex.sub(regex_act6, r'<กระทำ6>\1</กระทำ6>', read_text)
    read_text = matches_act6
    matches_act8_f = regex.sub(regex_act8_f, r'<กระทำ8_ผิด>\1</กระทำ8_ผิด>', read_text)
    read_text = matches_act8_f
...
def tag_location(read_text):
    ...
    regex_province2 = (r"(((?<!ผว|\.)จ\.|จังหวัด|(?<!\.)จว.)\s?(" + list_str_p + "){e<=1}|(กรุงเทพมหานคร|กรุงเทพ|กทม\.))")

    regex_province = (r"<จังหวัด_ผิด>(((?<!ผว|\.)จ\.|จังหวัด|(?<!\.)จว.)\s?(" + list_str_p + ")|(กรุงเทพมหานคร|กรุงเทพ|กทม\.))</จังหวัด_ผิด>")

    regex_country2 = (r"(((?<!ต่าง)ประเทศ)\s?(" + list_str_c + "){e<=1})")

    regex_country = (r"<ประเทศ_ผิด>(((?<!ต่าง)ประเทศ)\s?(" + list_str_c + "))</ประเทศ_ผิด>")
    ...
    regex_road2 = (r"((?!ถนนใกล้)((?<!\.)ถ\.|ถนน)\s?(" + list_str_r + "){e<=1}|(ถนน\s?ตก\s))")

    regex_road = (r"<ถนน_ผิด>((?!ถนนใกล้)((?<!\.)ถ\.|ถนน)\s?(" + list_str_r + ")|(ถนน\s?ตก\s))</ถนน_ผิด>")

    regex_amphoe2 = (r"(((?<!นาย|ปลัด)อำเภอ|(?<!\.|ผ)อ\.)\s?(" + list_str_a + "){e<=1})")

    regex_amphoe = (r"<อำเภอ_ผิด>(((?<!นาย|ปลัด)อำเภอ|(?<!\.|ผ)อ\.)\s?(" + list_str_a + "))</อำเภอ_ผิด>")
    ...
    regex_tambon = (r"<ตำบล_ผิด>((ตำบล|(?<!\.)ต\.)(?![ก-๙]\.)(" + list_str_t + ")\s?)</ตำบล_ผิด>")

    regex_location0 = (
        r"((?!ซอยดังกล่าว|ซอยมัก|ซอยเล็ก|ซอยตาม|ซอยหอ|ซอยใกล้|ซอยหรือ|ซอยไป|ซอยมี|ซอยถัด|สน.ที่|ซอยริมถนน|ซอยข้าง|ซอยเข้า|ซอยเปลี่ยว|ซอยที่เกิดเหตุ|ซอยตัน|ซอยหลบ|ซอยแคบ|ซอยด้วย|ซอยไม่มีชื่อ|ซอยข้างบ้าน|สน.ออก)(ซอย|\sซ\.|(?<!ท|\.)สภ\.|สน\.)(\s?\d\d?|[ก-๙]{2,}(?=แล้ว|ชัก|ไป|สอบ|ได้|จะ|ดำเนิน|มายัง|ต่อไป|จับกุม|เพื่อ|กล่าว|ดำเนิน|และ|รับเเจ้ง|ได้รับ|รับตัว)"
        "|(?!ตาย|เพื่อ|ได้|ของ|ใกล้|แล้ว|ให้|แวะ)[ก-๙]{2,}\s?(\d?\d?))|(?!ย่านถนน|ย่านหลัง|ย่านชุมชน|ย่านซอย|ย่านนี้)(ย่าน)([ก-๙]{2,}(?=ตาย|หาเงิน|เพื่อ)|[ก-๙]{2,})(\s?\d?\d?)|(บ้านเลขที่|ห้องเลขที่|บ้านเช่าเลขที่)\s?(\d\d?\d?\d?)"
        "(/)?(\d?\d?\d?)|((หมู่|หมู่ที่|(?<!ชั้น|\.|สาว)\sม\.)\s?(\d\d?)))")
    regex_location = (
        r"(((?<=ที่</คำบ่งบอก4>|ใน</คำบ่งบอก4>|บริเวณ</คำบ่งบอก4>|คา</คำบ่งบอก4>)โรงแรม|(?<=ที่</คำบ่งบอก4>|ใน</คำบ่งบอก4>|บริเวณ</คำบ่งบอก4>|คา</คำบ่งบอก4>)โรงเรียน|(?<=ที่</คำบ่งบอก4>|ใน</คำบ่งบอก4>|บริเวณ</คำบ่งบอก4>|คา</คำบ่งบอก4>)โรงรับจำนำ|(?<=ที่</คำบ่งบอก4>|ใน</คำบ่งบอก4>|บริเวณ</คำบ่งบอก4>|คา</คำบ่งบอก4>)โรงงาน)(?!เถื่อน|ฟัง|เอง|ส่ง|ต่าง|เท่านั้น|ทั้ง|แห่งนี้|แต่|กับ|เพราะ|เป็น|ที่|ส่วน|สั่ง|แห่งเดียว|จะได้|เดียว|ได้สั่ง|ก็โดน|ใน|มี|ต่างหาก|ก็เห็น|ใกล้|ตนได้|และ|ว่า|ที่เกิดเหตุ)([ก-๙]{2,}(?=ชื่อ|ย่าน|ใน|ริม)|(?!จริง|ชื่อ|ดังกล่าว)[ก-๙]{2,}))")
    regex_location1 = (
        r"((?<=ที่</คำบ่งบอก4>|ใน</คำบ่งบอก4>|บริเวณ</คำบ่งบอก4>|คา</คำบ่งบอก4>)บ้านพัก|(?<=ที่|ใน|บริเวณ|คา)บ้านหลังหนึ่ง|(?<=ที่|ใน|บริเวณ|คา)บ้านแห่งหนึ่ง|(?<=ที่|ใน|บริเวณ|คา)บ้านเช่า)")
    regex_location2 = (
        r"((?<=ที่|ใน|บริเวณ|คา)คอนโดมิเนียม|(?<=ที่|ใน|บริเวณ|คา)คอนโด|(?<=ที่|ใน|บริเวณ|คา)แมนชั่น|(?<=ที่|ใน|บริเวณ|คา)อะพาร์ตเมนต์|(?<=ที่|ใน|บริเวณ|คา)หน้าผับ|(?<=ที่|ใน|บริเวณ|คา)ผับ|(?<=ที่|ใน|บริเวณ|คา)ห้องเช่า|(?<=ที่|ใน|บริเวณ|คา)ห้องแถว)")
    regex_location3 = (
        r"(((?<=ที่|ใน|บริเวณ|คา)หน้าร้าน|(?<=ที่|ใน|บริเวณ|คา)ร้าน)(กาแฟ[ก-๙]{2,}|ข้าว[ก-๙]{2,}|อาหาร[ก-๙]{2,}(?=ใน|ชื่อ|ย่าน)|อาหาร[ก-๙]{2,}|ซ่อม[ก-๙]{2,}(?=ดังกล่าว)|ซ่อม[ก-๙]{2,}|ขาย[ก-๙]{2,}(?=ชื่อ|และ|ที่อยู่)|ขาย[ก-๙]{2,}|ทำ[ก-๙]{2,}|เสริม[ก-๙]{2,}|ร้านน้ำ[ก-๙]{2,}|สะดวกซื้อ|เซเว่นอีเลฟเว่น|เซเว่น|กาแฟ|อาหาร|คาราโอเกะ|เฟอร์นิเจอร์|เคเอฟซี|พิซซ่า|ทอง))")

    regex_mall = (r"((?<!<ห้าง_ผิด>)(ห้างสรรพสินค้า|ห้าง)(" + list_str_m + "))")
    regex_mall2 = (r"((?!ห้างฉัตร)(ห้างสรรพสินค้า|ห้าง)((?!ร้าน|มอบ|ย่อง|ผวา|หุ้น|ดัง|เปิด|และ|ค้า|ได้|ซึ่ง|" + list_str_m + ")[ก-๙]{2,}))")

    regex_hos = (r"((?<!<โรงพยาบาล_ผิด>)(โรงพยาบาล|ร\.พ\.|รพ\.)(" + list_str_h + "))")
    regex_hos2 = (r"((โรงพยาบาล|ร\.พ\.|รพ\.)(?!เสียก่อน|และ|เกี่ยว|ไป|ก่อน|ยืนยัน|อื่น|จ่าย|เป็น|เอง|ทันที|หลาย|โทร|ได้|เพื่อ|ว่า|ให้การ|ตำบล|สต|แล้ว|ใน|ใส่|ขณะ|เดิน|ได้แล้ว|เขาก็|ใกล้|ก่อนหน้า|ดังกล่าว|" + list_str_h + ")([ก-๙]{2,}(?=หาสาเหตุ|อาการ|ทันที|โดย|อีก|เพื่อ|ในเวลา|และ|ไป)|[ก-๙]{2,}))")

    regex_university = (r"((?<!<มหาวิทยาลัย_ผิด>)(มหาวิทยาลัย|มหาลัย)(" + list_str_uni + "))")
    regex_university2 = (r"((มหาวิทยาลัย|มหาลัย)(?!แล้ว|ของรัฐ|ก่อน|อื่น|ยืนยัน|หลาย|ได้|เพื่อ|ตำบล|ชื่อดัง|ดังกล่าว|ที่เคย|ต้นสังกัด|ไม่|" + list_str_uni + ")([ก-๙]{2,}))")
    ...
    matches_location0 = regex.sub(regex_location0, r'<สถานที่>\1</สถานที่>', read_text)
    read_text = matches_location0
    matches_location = regex.sub(regex_location, r'<สถานที่>\1</สถานที่>', read_text)
    read_text = matches_location
    matches_location1 = regex.sub(regex_location1, r'<สถานที่>\1</สถานที่>', read_text)
    read_text = matches_location1
    matches_location2 = regex.sub(regex_location2, r'<สถานที่>\1</สถานที่>', read_text)
    read_text = matches_location2
    matches_location3 = regex.sub(regex_location3, r'<สถานที่>\1</สถานที่>', read_text)
    read_text = matches_location3
...
def tag_person(read_text):
    regex_name = (
        r"((นาย|นาง|นางสาว|น\.ส\.|นพ\.|อส\.|ส\.ต\.|ร\.ต\.อ\.|ร\.ต\.|ดร\.|ร\.อ\.|พ\.อ\.|จ\.ส\.ต\.|จ่าสิบตำรวจ|ส\.ต\.อ\.|สิบตำรวจเอก|ส\.ต\.ท\.|"
        ...
        "ร้อยตำรวจโท|ร\.ต\.ท\.|ร้อยตำรวจตรี|ร\.ต\.ต\.|พล\.ต\.|ส\.ต\.|เด็กชาย|ด\.ช\.|เด็กหญิง|ด\.ญ\.|พ\.ต\.ท)\s?([ก-๙]{2,20})\s(หรือ\s?[ก-๙]{2,20})\s((?!สารภาพ|พฤติกรรม|สัมภาษณ์|สัญชาติ|ก้มกราบ|คู่กรณี|น้องของ|พนักงาน|เกิดความ|เยาวชน|สะบัด|ดิ้น|ร่วมรัก|โมโห|"
        ...
        "ลุก|ส่ง|ออก|อาจ|อีก|เคย|เผย|เอา|จริง|หาย|แชท|แต่|และ|แอบ|เจ้า|มือ|เพื่อ|กลับ|ใช้|ให้|ได้|ไม่|ไล่|ไว้|ก็|จน|จะได้|ทน|มานี่|มีอาการ|มีหมาย|ไป|ใน|พบ|พาไป|พากลับ|พามา|นำ|ว่า|ยิง|หนี|ประสานมา|ชก|"
        ...
        "|นักธุรกิจ|มาอยู่|มาให้|มีอาชีพ|มาที่|มีประวัติ|มาแจ้ง|มากโดย|มาเจรจา)[ก-๙]{2,20}|ชาววัง|หัสยา สัมมา|อาจหาญ|จะนู|กำลังรัมย์|พานิชอัตรา|มาสะอาด|กลับอินทร์|ไล่คง|ณ นคร|ณ อยุธยา|สุนทรกุล ณ ชลบุรี|สุคนธาภิรมย์ ณ พัทลุง|รัตนดิลก ณ ภูเก็ต|"
    ...
    regex_name1 = (
        r"((?!นายอำเภอ|อส\.ตร\.|นายุ|นายแบบ|นายาว|ด.ต.ปืน|นายช่าง|นายทะเบียน|เด็กชายที่|นายกรัฐ|นายกองค์การ|ร.ต.ท.ผ่าน|นายกเทศมนตรี|นายกสภา"
        "|นายกแพทย|นายกเทศบาล|นายเก่า|นายสิบ|นายดาบ|นายพลนักสืบ|ด.ต.ยิง|นายอย|นายทหาร|นายสถานี|นางเงือก|ทนายมาดำ|ทนายเข้า|นายเข้า|นางนกต่อ|พ.ต.ท.ซิ่งกระบะ|ทันตแพทย์ชื่อ|ทันตแพทย์หลง|"
        ...
        "นายมีความ|นายไม่)(?<!<คน2>|เจ้า|เจ้ากรรม|ท|ทำ|สิบ|ร้อย|พัน|หมื่น|แสน|ล้าน|ทุก|อีก|ตำรวจ|ทหาร|หนึ่ง|สอง|สาม|สี่|รอง|นิ้ว|ผีสาง|\s\d\d?\s)(นาย|สมเด็จพระนาง|"
        ...
        "พล\.อ\.ต\.|พลอากาศตรี|พล\.อ\.ท\.|พลอากาศโท|พล\.อ\.อ\.|พลอากาศเอก|พล\.อ\.|พลเอก|พล\.ท\.|พลโท|พลตรี|พล\.ต\.อ\.|พลตำรวจเอก|"
        "พลตำรวจโท|พล\.ต\.ท\.|พลตำรวจตรี|พล\.ต\.ต\.\s?|พ\.ต\.อ\.|พันจ่าอากาศ|พันตำรวจเอก|พันตำรวจโท|พ\.ต\.ท\.|พันตำรวจตรี|พ\.ต\.ต\.|ร้อยตำรวจเอก|"
        "ร้อยตำรวจโท|ร\.ต\.ท\.|ร้อยตำรวจตรี|ร\.ต\.ต\.|พล\.ต\.|ส\.ต\.|เด็กชาย|ด\.ช\.|เด็กหญิง|ด\.ญ\.|อส\.|พ\.ต\.ท)\s?(?!ข่มขืน|ได้ส่ง|บนรถ|ตัดใจ|จะพยายาม|หาย|จับ|มาเพราะ|เพื่อ|ขาย|ออก|ดู|ทั้ง|ผู้|ราย|อีก|"
        "อายุ|อย่าง|วัย|เป็น|คน|ตาย|มี|ดังกล่าว|ให้การ|มาก่อ|และ)([ก-๙]{2,30}(?=โดน|นั้น|ไม่|ผู้|ถ่าย|และ|เป็น|ด้วย|หรือ|ที่|จึง|ทำร้าย|ไป|ได้|เปิด|ให้|มาพร้อม|ว่า|อีกที|ค้าง|พร้อม|ระบุ|ให้การ)|[ก-๙]{2,30})(\s((?!ข่มขืน|บนรถ|สารภาพ|พฤติกรรม|สัมภาษณ์|สัญชาติ|ก้มกราบ|คู่กรณี|น้องของ|พนักงาน|เกิดความ|เยาวชน|สะบัด|ดิ้น|ร่วมรัก|โมโห|"
        "ยังอยู่|หลานของ|หลานชาย|หลานสาว|ตัดสิน|พยายาม|ชาวบ้าน|ชาว|พี่|พ่อ|ลูก|ลูก|หลบ|อยู่ใน|เพื่อน|แม่|คิดตัด|กล่าว|ทำงาน|น่าจะ|ปล่อย|นามสมมุติ|ภายใน|ภายนอก|กระทำ|แสดงตัว|ดูแล|ม่าย|"
        "พร้อม|จาก|จับ|ห้อย|โดย|เพิ่ง|ปั่น|นั้น|สวม|ด้วย|เบื้อง|ถึง|เล่า|ฟัง|กำลัง|ทราบ|ถือ|เริ่ม|บอก|แล้ว|ยกมือ|อดีต|ลง|ติดเชื้อ|ตกใจ|ทำร้าย|ใช่|จำเลย|ตั้ง|แต่ง|อย่าง|ย้อน|ข้อหา|บ่าย|เช้า|กลางวัน|ดึก|"
        "ภรรยา|มารดา|ซึ่ง|ญาติ|ตื่น|นั่ง|บิดา|เสียชีวิต|ย้าย|ระบุ|วิ่ง|หรือ|สามี|อายุ|อ้าง|เข้า|เปิด|เป็น|เห็น|กับ|ขณะ|ขับ|ขี่|จึง|จู่|ตอบ|ถูก|ทาง|ที่|ผู้|พัก|ยอม|ยัง|รับ|รุม|บริเวณ|ถอด|รอดคดี|น้อง|น้องสาว|เสพยา|โยน|"
        "เซล้ม|กลัว|โทร|ทั้งหมด|ทั้งสอง|ปรากฏ|ล้ม|กระโดด|แฟน|ก่อเหตุ|มากนัก|ค้างค่า|ตรวจค้น|เบื้องต้น|ประกอบ|ส่วน|ฟ้อง|ยื่น|ฐานะ|ตาม|จำนวน|เกลี้ยกล่อม|ขัดขวาง|ซุกซ่อน|บาดเจ็บ|อยู่บ้าน|เพียงลำพัง|หลอก|"
        ...
        "มีอาชีพ|มาที่|มีประวัติ|สภาพจิต|มาแจ้ง|มากโดย|เจอที่|มาเจรจา|ลวนลาม|ชั้น|สารวัตร)[ก-๙]{2,25}\s|[ก-๙]{2,25}\,|[ก-๙]{2,25}\”|ชาววัง|หัสยา สัมมา|อาจหาญ|จะนู|กำลังรัมย์|พานิชอัตรา|มาสะอาด|กลับอินทร์|ไล่คง|ณ นคร|ณ อยุธยา|สุนทรกุล ณ ชลบุรี|สุคนธาภิรมย์ ณ พัทลุง|รัตนดิลก ณ ภูเก็ต|"
        "ณ กาฬสินธุ์|ณ จัมปาศักดิ์|ณ เชียงใหม่|ณ เชียงตุง|ณ ตะกั่วทุ่ง|ณ ถลาง|ณ น่าน|ณ บางช้าง|ณ ป้อมเพชร|ณ พัทลุง|ณ พิศณุโลก|ณ มโนรม|ณ มหาไชย|ณ ราชสีมา|"
    ...
    regex_name = (
        r"<คน>((?!นายอำเภอ|อส\.ตร\.|นายุ|นายแบบ|นายาว|ด.ต.ปืน|นายช่าง|นายทะเบียน|เด็กชายที่|นายกรัฐ|นายกองค์การ|ร.ต.ท.ผ่าน|นายกเทศมนตรี|นายกสภา"
        "|นายกแพทย|นายกเทศบาล|นายเก่า|นายสิบ|นายดาบ|นายพลนักสืบ|ด.ต.ยิง|นายอย|นายทหาร|นายสถานี|นางเงือก|ทนายมาดำ|ทนายเข้า|นายเข้า|นางนกต่อ|พ.ต.ท.ซิ่งกระบะ|ทันตแพทย์ชื่อ|ทันตแพทย์หลง|"
        "นายมาดำ|นางแบบ|นายกต่อ|นายหนึ่งได้|นางเลิ้ง|นางงาม|นางนอน|นายจ้าง|นางรอง|นายทุน|นายกสมาคม|นายเป้เพื่อน|ร.ต.ท.วัย|นายเตรียม|ด.ต.นอก|ด.ต.ตาม|ร.ต.อ.เพื่อนสนิท|นายและเพื่อน|"
        "นายยก|ด.ต.หึงโหด|นายต่าง|นายทั่วประเทศ|นายพร้อมดูแล|นายแพทย์รวม|ทนายเผย|นายสิบทหาร|ด.ต.คู่|ด.ต.ดับ|ด.ต.โรงพัก|ด.ต.รายนี้|นางบวช"
        "|นายกฯ|นายช่วย|นายความ|นายทุน|นายพราน|นายตรวจ|นางเอก|นายรอง|นายหน้า|นายตำรวจ|ทนายความ|ดร\.หนุ่ม|ส\.ต\. มา|นายทิ้ง|นายกรัฐมนตรี|ทนายเปรมชัย|"
        ...
        "พล\.อ\.ต\.|พลอากาศตรี|พล\.อ\.ท\.|พลอากาศโท|พล\.อ\.อ\.|พลอากาศเอก|พล\.อ\.|พลเอก|พล\.ท\.|พลโท|พลตรี|พล\.ต\.อ\.|พลตำรวจเอก|"
        "พลตำรวจโท|พล\.ต\.ท\.|พลตำรวจตรี|พล\.ต\.ต\.|พ\.ต\.อ\.|พันจ่าอากาศ|พันตำรวจเอก|พันตำรวจโท|พ\.ต\.ท\.|พันตำรวจตรี|พ\.ต\.ต\.|ร้อยตำรวจเอก|"
        "ร้อยตำรวจโท|ร\.ต\.ท\.|ร้อยตำรวจตรี|ร\.ต\.ต\.|พล\.ต\.|ส\.ต\.|เด็กชาย|ด\.ช\.|เด็กหญิง|ด\.ญ\.|อส\.|พ\.ต\.ท)\s?(?!จะพยายาม|หาย|จับ|มาเพราะ|เพื่อ|ขาย|ออก|ดู|ทั้ง|ผู้|ราย|อีก|"
        "อายุ|อย่าง|วัย|เป็น|คน|ตาย|มี|ดังกล่าว|ให้การ|มาก่อ|และ)([ก-๙]{2,30}(?=โดน|ไม่|ผู้|ถ่าย|และ|เป็น|ด้วย|หรือ|ที่|จึง|ทำร้าย|ไป|ได้|เปิด|ให้|มาพร้อม|ว่า|อีกที|ค้าง|พร้อม|ระบุ|ให้การ)|[ก-๙]{2,30})(\s((?!สารภาพ|พฤติกรรม|สัมภาษณ์|สัญชาติ|ก้มกราบ|คู่กรณี|น้องของ|พนักงาน|เกิดความ|เยาวชน|สะบัด|ดิ้น|ร่วมรัก|โมโห|"
        "ยังอยู่|หลานของ|หลานชาย|หลานสาว|ตัดสิน|พยายาม|ชาวบ้าน|ชาว|พี่|พ่อ|ลูก|ลูก|หลบ|อยู่ใน|เพื่อน|แม่|คิดตัด|กล่าว|ทำงาน|น่าจะ|ปล่อย|นามสมมุติ|ภายใน|ภายนอก|กระทำ|แสดงตัว|ดูแล|ม่าย|"
        "พร้อม|จาก|จับ|ห้อย|โดย|เพิ่ง|ปั่น|นั้น|สวม|ด้วย|เบื้อง|ถึง|เล่า|ฟัง|กำลัง|ทราบ|ถือ|เริ่ม|บอก|แล้ว|ยกมือ|อดีต|ลง|ติดเชื้อ|ตกใจ|ทำร้าย|ใช่|จำเลย|ตั้ง|แต่ง|อย่าง|ย้อน|ข้อหา|บ่าย|เช้า|กลางวัน|ดึก|"
        "ภรรยา|มารดา|ซึ่ง|ญาติ|ตื่น|นั่ง|บิดา|เสียชีวิต|ย้าย|ระบุ|วิ่ง|หรือ|สามี|อายุ|อ้าง|เข้า|เปิด|เป็น|เห็น|กับ|ขณะ|ขับ|ขี่|จึง|จู่|ตอบ|ถูก|ทาง|ที่|ผู้|พัก|ยอม|ยัง|รับ|รุม|บริเวณ|ถอด|รอดคดี|น้อง|น้องสาว|เสพยา|โยน|"
        "เซล้ม|กลัว|โทร|ทั้งหมด|ทั้งสอง|ปรากฏ|ล้ม|กระโดด|แฟน|ก่อเหตุ|มากนัก|ค้างค่า|ตรวจค้น|เบื้องต้น|ประกอบ|ส่วน|ฟ้อง|ยื่น|ฐานะ|ตาม|จำนวน|เกลี้ยกล่อม|ขัดขวาง|ซุกซ่อน|บาดเจ็บ|อยู่บ้าน|เพียงลำพัง|หลอก|"
        ...
        "นางสาว|รออยู่|มาประกอบ|ต่อพนักงาน|เเละจำเลย|วัย|ต่อศาล|มาดำ|หนึ่งใน|แน่นิ่ง|ฐานฆ่า|ปกติจะ|โกง|ทำให้|ยืนยัน|ตนเอง|สภาพศพ|มาประมาณ|มีคำสั่ง|ของทาง|อยู่กิน|รองศาสตราจารย์|รองสารวัตร"
        "|มีสีหน้า|ตำรวจ|เลิกงาน|เตะ|มาทำ|ซ้ำอีก|สังเกต|ต่อมา|มาขอ|มาจาก|หัวหน้าทีม|รวมมูลค่า|มาตั้ง|รักกัน|มาห่าง|ระแวง|ร้องไห้|มาหา|มารับ|มามอบ|มาก่อน|มาคุม|มาพัก|บุกรุก|นักธุรกิจ|มาอยู่|มาให้|"
        "มีอาชีพ|มาที่|มีประวัติ|สภาพจิต|มาแจ้ง|มากโดย|มาเจรจา|ลวนลาม|ชั้น|สารวัตร)[ก-๙]{2,25}\s|[ก-๙]{2,25}\,|[ก-๙]{2,25}\”|ชาววัง|หัสยา สัมมา|อาจหาญ|จะนู|กำลังรัมย์|พานิชอัตรา|มาสะอาด|กลับอินทร์|ไล่คง|ณ นคร|ณ อยุธยา|สุนทรกุล ณ ชลบุรี|สุคนธาภิรมย์ ณ พัทลุง|รัตนดิลก ณ ภูเก็ต|"
        "ณ กาฬสินธุ์|ณ จัมปาศักดิ์|ณ เชียงใหม่|ณ เชียงตุง|ณ ตะกั่วทุ่ง|ณ ถลาง|ณ น่าน|ณ บางช้าง|ณ ป้อมเพชร|ณ พัทลุง|ณ พิศณุโลก|ณ มโนรม|ณ มหาไชย|ณ ราชสีมา|"
    ...
    # add tag person
    regex_name = (
        r'(คนร้าย|กำนัน|คนเสียหาย|ผู้ร้าย|ผู้ต้องหา|ผู้ก่อเหตุ|ผู้เสียหาย|ผู้ตาย|ผู้เสียชีวิต|โจรใต้|โจร(?!กรรม)|เด็กหญิง|ตำรวจ|ตร\.|ด\.ญ\.|'
        'จนท\.|เหยื่อ|มือปืน)')
    ...
    regex_name3 = (
        ...
        "ณ กาฬสินธุ์|ณ จัมปาศักดิ์|ณ เชียงใหม่|ณ เชียงตุง|ณ ตะกั่วทุ่ง|ณ ถลาง|ณ น่าน|ณ บางช้าง|ณ ป้อมเพชร|ณ พัทลุง|ณ พิศณุโลก|ณ มโนรม|ณ มหาไชย|ณ ราชสีมา|"
        "ณ ร้อยเอ็จ|ณ ลำพูน|ณ วิเชียร|ณ วังขนาย|ณ สงขลา|ณ หนองคาย|ณ อุบล|โกมารกุล ณ นคร|พรหมสาขา ณ สกลนคร|ภวภูตานนท์ ณ มหาสารคาม|ประทีป ณ ถลาง))?)")
    matches_name3 = regex.sub(regex_name3, r'<คน>\1</คน>', read_text)
    read_text = matches_name3
...
def tag_adverb(read_text):
    ...
    regex_specify4 = (r"((?<!ต\.|อ\.)ที่(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|"
                      "มืด|ลาน|จอด|เซเว่น|ปั้ม|สำนัก|สาธารณะ|ลุ่ม|ริม|ช่อง|ร่องน้ำ|ท่า|บ่อ|หน้า|คุก|ศาล|มหาวิทยาลัย|ประเทศ|จังหวัด|จ\.|"
                      "อำเภอ|อ\.|ตำบล|ต\.|เขต|แขวง|รพ\.|สน\.|สภ\.|ข้าง|ปากซอย|ศาลา|ตึก|คลอง|ชุมชน|บริษัท|หาด|เรือนจำ|ลำห้วย|"
                      "รีสอร์ต|ป้อม|ป่า|เคหะ|คอนโด|แมนชั่น|อะพาร์ตเมนต์|ผับ|กลาง|ถนน|ตลาด|กุฏิ)|"
                      "(?<!ต\.|อ\.)คา(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|ลาน|จอด|เซเว่น|ปั้ม|สำนัก|"
                      "สาธารณะ|ลุ่ม|ริม|ช่อง|ร่องน้ำ|ท่า|บ่อ|หน้า|คุก|ศาล|มหาวิทยาลัย|ประเทศ|จังหวัด|จ\.|อำเภอ|อ\.|ตำบล|ต\.|เขต|แขวง|รพ\.|"
                      "สน\.|สภ\.|ข้าง|ปากซอย|ศาลา|ตึก|คลอง|ชุมชน|บริษัท|หาด|เรือนจำ|ลำห้วย|รีสอร์ต|ป้อม|ป่า|เคหะ|คอนโด|แมนชั่น|อะพาร์ตเมนต์|"
                      "ผับ|กลาง|ถนน|ตลาด|กุฏิ)|(?<!ต\.|อ\.)บริเวณ(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|ลาน|"
                      "จอด|เซเว่น|ปั้ม|สำนัก|สาธารณะ|ลุ่ม|ริม|ช่อง|ร่องน้ำ|ท่า|บ่อ|หน้า|คุก|ศาล|มหาวิทยาลัย|ประเทศ|จังหวัด|จ\.|อำเภอ|อ\.|ตำบล|ต\.|"
                      "เขต|แขวง|รพ\.|สน\.|สภ\.|ข้าง|ปากซอย|ศาลา|ตึก|คลอง|ชุมชน|บริษัท|หาด|เรือนจำ|ลำห้วย|รีสอร์ต|ป้อม|ป่า|เคหะ|คอนโด|แมนชั่น|"
                      "อะพาร์ตเมนต์|ผับ|กลาง|ถนน|ตลาด|กุฏิ)|(?<!ต\.|อ\.)ใน(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|"
                      "ลาน|จอด|เซเว่น|ปั้ม|สำนัก|สาธารณะ|ลุ่ม|ริม|ช่อง|ร่องน้ำ|ท่า|บ่อ|หน้า|คุก|ศาล|มหาวิทยาลัย|ประเทศ|จังหวัด|จ\.|อำเภอ|อ\.|ตำบล|ต\.|"
                      "เขต|แขวง|รพ\.|สน\.|สภ\.|ข้าง|ปากซอย|ศาลา|ตึก|คลอง|ชุมชน|บริษัท|หาด|เรือนจำ|ลำห้วย|รีสอร์ต|ป้อม|ป่า|เคหะ|คอนโด|แมนชั่น|"
                      "อะพาร์ตเมนต์|ผับ|กลาง|ถนน|ตลาด|กุฏิ))")
    ...
    matches_specify4 = regex.sub(regex_specify4, r'<คำบ่งบอก4>\1</คำบ่งบอก4>', read_text)
    read_text = matches_specify4
    return read_text
