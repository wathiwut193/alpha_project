...
def tag_action(read_text):
    ...
    regex_act1 = (r"((?!ยิง\|?ตัว|ฆ่า\|?ตัว|ฟัน\|?กำไร)((?<!ไม่\|)ยิง|(?<!ยา\|)ฆ่า|ฆาตกรรม|"
                  "สังหาร|กระทำชำเรา|อนาจาร|ข่มขืน|ขืนใจ|ฉุด|ฟัน|แทง))")
    # regex_act2 = (r"((" + list_str_act2 + "))")
    regex_act2_f = (r"((ฆ่าตัวตาย|ยิงตัวเอง|ปลิดชีพตัวเอง|ปลิดชีวิตตัวเอง|จบชีวิต|อัตวินิบาตกรรม){e<=1})")
    regex_act2 = (
        r"<กระทำ2_ผิด>((\|)(ฆ่าตัวตาย|ยิงตัวเอง|ปลิดชีพตัวเอง|ปลิดชีวิตตัวเอง|จบชีวิต|อัตวินิบาตกรรม))</กระทำ2_ผิด>")
    # regex_act3 = (r"((" + list_str_act3 + "))")
    # regex_act3 = (r"(((?<!\.\|)สอบสวน|ดำเนินการคดี|ดำเนินคดี|ลาดตระเวน|แถลงการณ์|ลงพื้นที่|ยึดทรัพย์|สอบปากคำ|"
    #     "รับแจ้ง|ตรวจสอบ|เปิดเผย|ตรวจค้น|ส่งฟ้อง|วิสามัญ|เอาผิด|บุกรวบ|เผยว่า|ตามล่า|คุมตัว|ควบคุม|"
    #     "(?<!ชุด\|?|ฝ่าย\|?|ตำรวจ\|?)สืบสวน|ติดตาม|นำแถลง|รายงาน|วางแผน|จับกุม|(?<!หมาย\|?)จับ|"
    #     "นำตัว|(?<!ส)ระบุ|ปะทะ|เก็บ|บุก|(?<!เร่ง\|?)รวบ|(?<!เวลา\|?)ไล่|คุม|(?<!ไม่\|?|หาก\|?)พบ))")
    # update 3/5/19
    regex_act3 = (r"(((?<!\.\|)สอบสวน|ดำเนินการคดี|ดำเนินคดี|สอบปากคำ|ตรวจสอบ|ตรวจค้น|"
                  "วิสามัญ|เอาผิด|คุมตัว|ควบคุม|(?<!ชุด\|?|ฝ่าย\|?|ตำรวจ\|?)สืบสวน|จับกุม|(?<!หมาย\|?)จับ|คุม))")
    regex_act4 = (r"(((?!พาล|พานิช|พาย|พาหนะ)(?<!พก|พัด|สะ|วิ|ยาน|เฉ|บูร|อัม|อ|พิ)พา))")
    # regex_act5 = (r"((?!ให้การ\|?ปฏิเสธ)(" + list_str_act5 + "))")
    regex_act5_f = (r"((?!ให้การ\|?ปฏิเสธ)(ให้ปากคำ|ให้การ){e<=1})")
    regex_act5 = (r"<กระทำ5_ผิด>((\|)(?!ให้การ\|?ปฏิเสธ)(ให้ปากคำ|ให้การ))</กระทำ5_ผิด>")
    ...
    regex_act8_f = (r"((แจ้งความ|ร้องทุกข์|ฟ้องร้อง|ร้องเรียน|แจ้งคดี){e<=1})")
    regex_act8 = (r"<กระทำ8_ผิด>((\|)((แจ้งความ|ร้องทุกข์|ฟ้องร้อง|ร้องเรียน|แจ้งคดี)))</กระทำ8_ผิด>")
    # add tag
...
def tag_adverb(read_text):
    ...
    regex_specify4 = (r"((ภายใน|บริเวณ))")
    regex_specify4 = (r"(ที่(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|"
                      ...
                      "คา(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|ลาน|จอด|เซเว่น|ปั้ม|สำนัก|"
                      ...
                      "ผับ|กลาง|ถนน|ตลาด|กุฏิ)|บริเวณ(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|ลาน|"
                      ...
                      "อะพาร์ตเมนต์|ผับ|กลาง|ถนน|ตลาด|กุฏิ)|ใน(?=เกิดเหตุ|บ้าน|โรง|ห้อง|ร้าน|ซอย|เมือง|หมู่|ย่าน|พื้น|มัสยิด|วัด|ห้าง|สวน|สถานี|แหล่ง|มืด|"
